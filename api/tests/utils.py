...
class TestUtils(APITestCase):
    ...
    def create_favorite_product_if_not_exists(customer, identifier=1, product_id='q1w2e3r4', product_title='Test', product_price=1000.00, product_image='http://google.com/image'):
        if CustomerFavoriteProduct.objects.filter(product_id=product_id).exists():
            return CustomerFavoriteProduct.objects.filter(product_id=product_id)

        return CustomerFavoriteProduct.objects.create(
            id=identifier,
            customer=customer,
